...
- The breadcrumb hook returns segment names (e.g. `dashboard`, `attendance`, `participants`, `workshops`, `feedback`, `enrollees`, `staff`). These names **are** the translation keys.
...
- Use **singular** when the context is about one item (e.g. selecting, searching, or displaying one event).
- Use **plural** when the context is about many items (e.g. a list heading, "no items found" for an empty list).

| Context                        | Key                 | Example Value     |
| ------------------------------ | ------------------- | ----------------- |
| Placeholder: select one event  | `select-event-dots` | "Select event..." |
| Placeholder: search one event  | `search-event-dots` | "Search event..." |
| Empty state: no event selected | `no-event-found`    | "No event found"  |
