...
- The **general** folder should include purely generic, reusable UI that does not belong to any feature.
- `_app` hosts app-level components — global providers, top-level layout, and any app-wide shell or wrapper components.

## 2 — `src/contexts`

- One file per React context, named `<thing>-provider/`.
- Each exposes `<Thing>Provider` and `use<Thing>()`.
- Hold only _cross-feature_ state (theme, language, app-wide UI state, etc.).

## 3 — `src/hooks`

- A **folder = feature**. Each folder resembles a feature.
...
- Flat JSON files per supported language (e.g. `en.json`, `ar.json`).
- Keys use `kebab-case`.
- Add / modify keys in **all** locale files within the same PR.
...
- Routes are typically **shallow** (1–2 levels deep); deep nesting is discouraged since exhibit UIs rarely have complex navigation.

## 7 — `src/services`

- `API.ts` centralises/reuses config.
- Folder per **feature** approach.
- `.api.ts` → Concrete network calls built on utilising `API.ts`. _Only_ async functions and Feature type declaration here.
